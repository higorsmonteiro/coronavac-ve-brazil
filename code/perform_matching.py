...
# ----> load cpfs with irregular vaccine information on d1 and d2
cpf_wrong_d1 = pd.read_parquet(os.path.join("output", "data", "CPF_IRREGULAR_DATA_D1.parquet"))
cpf_wrong_d2 = pd.read_parquet(os.path.join("output", "data", "CPF_IRREGULAR_DATA_D2.parquet"))
if dose=="DATA D1":
    fschema = fschema[~fschema["CPF"].isin(cpf_wrong_d1["cpf"])]
elif dose=="DATA D2":
    fschema = fschema[~fschema["CPF"].isin(cpf_wrong_d2["cpf"])]
...
m_obj.generate_survival_info_v2(output_folder, date_str=dose, seed=seed, delay_vaccine=delay_vaccine) 
